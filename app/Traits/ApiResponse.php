...
trait ApiResponse
{
  ...
  */
    ...
    {
        if ($format === 'csv') {
            return $this->exportDataToCsv($data, $resource);
        }
    ...
    protected function exportDataToCsv($data, $resource): StreamedResponse
    {
        $csvData = $resource::collection($data)->resolve();
        $headers = [
        ...
        $callback = function () use ($csvData) {
            ...
            // Insert header row
            if (count($csvData) > 0) {
                fputcsv($file, array_keys((array) $csvData[0]));
            }
            // Insert data rows
